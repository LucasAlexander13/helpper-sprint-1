...
function estruturarTabela(lista: Array<Pessoa>): string {
...
function criarTabela(lista: Array<Pessoa>) {
    let tabela: string = estruturarTabela(lista) + adicionarDados(lista);
...
function apagarItem() {
    ...
    const idInserido: number = Number((document.getElementById("fApagarItem") as HTMLInputElement).value);
    // verifica se o id inserido existe, retornando -1 caso contrário
    const indice: number = lista.findIndex(lista => lista.id === idInserido);
    ...
    if (indice !== idInexistente) {
        ...
        lista = lista.filter((pessoa: Pessoa) => pessoa.id !== idInserido);
        criarTabela(lista);
    }
    else {
...
function alterarItem() {
    // declaração de constantes a partir do getElementById do formulário e do id
    const formulario: HTMLFormElement = document.getElementById("formulario") as HTMLFormElement;
    const idInserido: number = Number((document.getElementById("fAlterarItem") as HTMLInputElement).value);
    // recebe os valores a partir do preenchimento do formulário pelo HTMLInputElement
    const novoNome: string = (document.getElementById("fAlterarNome") as HTMLInputElement).value;
    const novaBio: string = (document.getElementById("fAlterarBio") as HTMLInputElement).value;
    // verifica se o id inserido existe, retornando -1 caso contrário
    const indice: number = lista.findIndex(lista => lista.id === idInserido);
    const idInexistente: number = -1;
    ...
    if (indice !== idInexistente) {
        ...
        if (novoNome) lista[indice].name = novoNome;
        if (novaBio) lista[indice].bio = novaBio;
